...
const reactPaths = pathMatches.map((path, index) => {
  // Extract id, title, and d attributes (id can be anywhere in the path element)
  const idMatch = path.match(/id="([^"]*)"/);
  const titleMatch = path.match(/title="([^"]*)"/);
  const dMatch = path.match(/d="([^"]*)"/);
  
  if (!idMatch || !dMatch) {
    console.log('Skipping path without id or d:', path.substring(0, 100) + '...');
    return null;
  }
  ...
  // Convert id to camelCase for React (handle "PATH 4" -> "path-4")
...
// Save to file
fs.writeFileSync('front-body-svg-fixed.txt', frontBodySVGContent);
console.log('Saved complete frontBodySVG to front-body-svg-fixed.txt');
